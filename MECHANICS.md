...
| 1/3/5個ロール、Luckが多数個率を乗算 | `P(5)=0.004·luck, P(3)=0.036·luck` | 🟡(基礎確率) |
| 宝箱は**所持品のレベルアップのみ**(新規は出ない) | `upgradable` = owned only | ✅ |
| 適用不能ロールは金へ変換、Greed乗算 | `GOLD_BASE30 + 15/item + minute` | 🟡(数値) |
| ミニボスが宝箱ドロップ | boss死亡→Pickup(Chest) | ✅ |
...
- 床の光源(ろうそく等)は「約40秒ごとにプレイヤー付近へフィールドドロップ湧き
  (金貨5/御饌3/磁石1/爆弾1の重み)」として機能等価に実装(v1.1.1)。加えて通常敵が
  2%×Luck で金貨(1〜3文)をドロップ — 原作の「敵からの金貨」系に対応
